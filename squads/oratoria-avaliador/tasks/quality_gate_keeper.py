...
def aggregate_gates(
    *,
    ...
    try:
        logger.info("operation_started", extra={"task": "quality_gate_keeper", "evaluation_id": evaluation_id})
        ...
        logger.info("operation_completed", extra={"task": "quality_gate_keeper", "verdict": result.get("verdict", "unknown")})
        return result
    except (KeyError, TypeError, ValueError) as e:
        logger.error("operation_failed", exc_info=True, extra={"task": "quality_gate_keeper", "error_type": type(e).__name__})
        return {
            "gate": "G_FINAL_RELEASE",
            "schema_version": "1.0.0",
            "evaluation_id": evaluation_id,
            "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
            "verdict": "INCOMPLETE",
            "release_to_user": False,
            "gate_states": {},
            "critical_fails": [],
            "unknowns": [],
            "waiver": waiver,
            "error": {"type": type(e).__name__, "message": str(e)},
            "next_action": "WAIT → erro interno no quality_gate_keeper",
        }
